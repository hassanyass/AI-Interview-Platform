...
- **How to actually host the agent worker**, given Render's free tier structurally cannot do it, and now also ruled out running it as a subprocess inside the backend's own container (real measured memory alone already exceeds the free tier's 512MB before any live session — see the follow-up research above): accept the free-but-fragile Render web-service-disguise + external uptime-pinger workaround on its own dedicated free service (the two bugs are already fixed), self-manage a genuinely-free VM (Google Cloud e2-micro is the more reliably-available option found; Oracle Cloud Always Free is real but has reported capacity/provisioning problems for the ARM shape — its smaller AMD micro-VM shape wasn't independently re-verified this session), or pay $7/month for a real Render Background Worker.
