    ...
    <div className="min-h-screen w-full bg-[#f6f8fb] text-foreground">
      ...
      <main className="mx-auto grid min-h-[calc(100vh-64px)] max-w-[1440px] grid-cols-1 gap-4 p-4 sm:p-6 lg:h-[calc(100vh-64px)] lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-6 lg:overflow-hidden">
        <section className="flex min-w-0 flex-col gap-4 lg:min-h-0 lg:overflow-hidden">
          ...
          </div> : <article className="min-h-0 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-[0_2px_8px_rgba(15,23,42,0.04)]"><div className="border-b border-slate-100 px-5 py-4 sm:px-7"><div className="flex flex-wrap items-center gap-2 text-xs"><span className="rounded-md bg-sky-50 px-2 py-1 font-semibold capitalize text-sky-700">{question?.difficulty || "Getting ready"}</span>{question?.competency && <span className="rounded-md bg-slate-100 px-2 py-1 text-slate-600">{question.competency}</span>}{question?.time_budget_minutes && <span className="text-slate-400">{question.time_budget_minutes} min</span>}</div></div>{isCompleted ? <div className="flex min-h-64 flex-col items-center justify-center gap-3 px-5 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600"><CheckCircle2 className="h-6 w-6" /></span><h2 className="text-2xl font-semibold text-slate-950">Interview complete</h2><p className="text-sm text-slate-500">Your result is being prepared.</p></div> : question ? <div className="space-y-6 px-5 py-6 sm:px-7"><div className="space-y-3"><h2 className="text-base font-semibold text-slate-950 sm:text-lg">Problem statement</h2><p className="whitespace-pre-line text-[15px] leading-7 text-slate-600">{question.problem_statement}</p></div></div> : <div className="flex min-h-64 items-center justify-center text-sm text-slate-500">Waiting for the interviewer...</div>}</article>}
