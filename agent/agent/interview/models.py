...
class InterviewPhase(str, enum.Enum):
    ...
    # WR-B (docs/section-pacing-architecture.md): candidate-visible,
    # clock-pausing stop between two HR-approved core sections. Reachable
    # only from BACKGROUND, only when another section remains (the last
    # section's completion still goes straight to CLOSING, unchanged).
    # Inert until WR-C actually transitions something into it.
...
class CandidateControlAction(str, enum.Enum):
    ...
    # WR-B: the candidate's explicit "I'm ready, continue" while in
    # WAITING_ROOM. No existing control maps to this. Handler logic (what
    # this actually does) is WR-C's job — state_machine.py only accepts it
    # as valid here, doesn't yet exist as a reachable path.
    PROCEED_TO_NEXT_SECTION = "PROCEED_TO_NEXT_SECTION"
...
class Question(BaseModel):
    ...
    competency: Optional[str] = None  # nullable — Phase 7A found ~32% of live HR-authored InterviewQuestion rows have no competency set
    ...
    # Phase 9D: HR-authored grading rubric, shape varies by section_type
    # (VERBAL: excellent/good/adequate/poor bands; CODING: time_complexity/
    # space_complexity/edge_cases/rubric; MCQ: explanation-only). Was
    # generated and validated since 9A but never reached the runtime
    # Question object until now -- see build_core_sections() in main.py.
...
# ─── Ordered Core-Question Section Progress (Phase 7B) ─────────────────────────
# Tracks "current position in an ordered, pre-approved (HR-authored) question
# list" for one InterviewSection. Core questions are immutable once loaded —
# never skipped, replaced, or reordered live (Verbal AI Follow-Up spec,
# reconstructed in docs/phase7-architecture.md). Fully separate from
# SectionProgress/background_progress/technical_progress above, which remain
# unmodified for legacy (pre-Phase-7) sessions.

class OrderedSectionProgress(BaseModel):
    section_type: str  # "VERBAL" for MVP; forward-compatible with CODING/MCQ (Phase 9)
    questions: List[Question] = Field(default_factory=list)
    current_index: int = 0
    completed: bool = False
    # WR-A (docs/section-pacing-architecture.md): this section's own time
    # budget, sourced from InterviewSection.config via /load's
    # SectionPayload. None for a legacy session or a section that predates
    # WR-A's publish-time requirement. Not consumed by anything yet — WR-C
    # wires this into controller._start_section_clock() at the waiting-room
    # boundary; until then it's carried through inertly.
    time_budget_minutes: Optional[int] = None
    # 7F-scoping addendum: whether the CURRENT question has actually been
    # posed to the candidate at least once. Closes the "adversarial
    # first-turn skip" gap — without this, nothing stopped a TRANSITION on
    # the very first turn from silently marking a never-asked question
...
# See docs/phase7-architecture.md 7B/7F: normal_min_seconds is a provisional
# estimate, not a measured value — 7F must sanity-check it against a real
# timed run.

class TimeTierThresholds(BaseModel):
    normal_min_seconds: int = 300   # >= this: "normal" — up to max_followups_per_question
    limited_min_seconds: int = 120  # >= this (and < normal): "limited" — 0-1 follow-ups
    # < limited_min_seconds: "very_limited" — 0 follow-ups, advance to next core question
...
class InterviewRuntimeContext(BaseModel):
    ...
    # Phase 7B: ordered, pre-approved (HR-authored) core-question sections,
